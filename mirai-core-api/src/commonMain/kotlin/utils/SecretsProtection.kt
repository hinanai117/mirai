...
public object SecretsProtection {
    ...
    init {
        ...
        if (tmbuffer.javaClass === ByteBuffer.allocate(1).javaClass) {
            val logger = MiraiLogger.Factory.create(SecretsProtection::class.java)
            logger.warning {
                "SECRET WARNING: 当前 JRE 实现没有为 `ByteBuffer.allocateDirect` 直接分配本地内存, 请更换其他 JRE."
            }
            logger.warning {
                "SECRET WARNING: Current JRE Implementation not using native memory for `ByteBuffer.allocateDirect`. Please use another JRE."
            }
        }
